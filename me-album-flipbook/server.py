...
def verify_token(token: Optional[str], role: str, version: int) -> bool:
    ...
    try:
        ...
        if token_role != role:
            return False
        if int(token_version) != int(version):
            return False
...
def ensure_demo():
    ...
    for i in range(1, 5):
        ...
        d.text((160, 362), 'Use the admin panel to upload a PDF or images.', fill=fg, font=mid)
        name = f'{i:02d}.jpg'
        img.save(PAGES_DIR / name, quality=92)
        names.append(name)
    save_manifest({'title': 'Album Preview', 'subtitle': 'Default admin password: umbrel', 'showCover': True, 'pages': names})
...
@app.post('/api/viewer/logout')
def viewer_logout(response: Response):
...
def api_meta(payload: MetaPayload, request: Request):
    ...
    if payload.showCover is not None:
        manifest['showCover'] = payload.showCover
...
def api_clear(request: Request):
    require_admin(request)
    clear_pages()
    save_manifest({'title': 'Album Preview', 'subtitle': 'Album cleared', 'showCover': True, 'pages': []})
    return {'ok': True}
...
async def upload_images(
    ...
    title: str = Form('Album Preview'),
    subtitle: str = Form('Uploaded images'),
    replace: bool = Form(True),
    showCover: bool = Form(True),
):
    require_admin(request)
    PAGES_DIR.mkdir(parents=True, exist_ok=True)
    ...
    save_manifest({'title': title, 'subtitle': subtitle, 'showCover': showCover, 'pages': pages})
...
async def upload_pdf(
    ...
    title: str = Form('Album Preview'),
    subtitle: str = Form('Uploaded PDF'),
    replace: bool = Form(True),
    showCover: bool = Form(True),
